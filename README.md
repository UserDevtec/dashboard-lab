# Dashboard Lab
[![Deploy to GitHub Pages](https://github.com/UserDevtec/dashboard-lab/actions/workflows/deploy.yml/badge.svg)](https://github.com/UserDevtec/dashboard-lab/actions/workflows/deploy.yml)

Dashboard Lab is een overzichtspagina voor GitHub Pages labs met status badges, snelle links en ruimte voor visuals.
Live demo: https://userdevtec.github.io/dashboard-lab

<img width="1846" height="1026" alt="image" src="https://github.com/user-attachments/assets/e451eeb5-5950-457b-b89e-69236759812f" />

## Features
- Cards met status badges per lab.
- Snelle links naar live sites en repositories.
- Visuele placeholders voor afbeeldingen per lab.
- Responsive layout met dezelfde stijl als turtle-lab.

## Gebruik
1) Open het dashboard.
2) Klik op een lab om de live site of repository te openen.

## Ontwikkelen
```bash
npm install
npm run dev
```
